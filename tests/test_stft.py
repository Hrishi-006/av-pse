...
def test_forward_shape() -> None:
    ...
    assert spec.shape == (2, 257, 373), f"Got {spec.shape}"
...
def test_round_trip_preserves_waveform() -> None:
    wav = torch.randn(2, 47648)
    spec = compute_stft(wav)
    recon = compute_istft(spec, length=47648)
    assert recon.shape == (2, 47648)
...
def test_handles_1d_input_with_batch_dimension() -> None:
    wav_1d = torch.randn(47648)
    spec = compute_stft(wav_1d)
    assert spec.shape == (1, 257, 373)


def test_device_follows_input() -> None:
    if torch.cuda.is_available():
        wav = torch.randn(2, 47648).cuda()
        spec = compute_stft(wav)
...
def test_frame_count_helper() -> None:
    assert get_stft_frame_count(47648, hop_length=128) == 373
...
def test_different_batch_sizes() -> None:
    for batch_size in [1, 4, 8]:
        ...
        spec = compute_stft(wav)
        assert spec.shape == (batch_size, 257, 373)
